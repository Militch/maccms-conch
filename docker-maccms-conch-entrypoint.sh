...
has_old=$(cat /var/www/html/application/data/config/quickmenu.txt | grep "[^/]*[^/]/*/admin/conch/theme")
if [ -z "$has_old" ]; then
    echo "" >> ./application/data/config/quickmenu.txt
    echo -e "海螺主题设置,/${MACCMS_ADMIN_FILENAME}/admin/conch/theme\c" >> ./application/data/config/quickmenu.txt
fi
